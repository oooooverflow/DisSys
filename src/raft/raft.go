...
func (rf *Raft) RequestVote(args RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.Term < rf.CurrentTerm || (rf.Role == 1 && rf.CurrentTerm == args.Term) {
		...
		if rf.VotedFor == -1 || rf.VotedFor == args.CandidateId {
			if args.LastLogIndex == 0 || len(rf.Logs) == 0 || rf.Logs[len(rf.Logs)-1].Term <= args.LastLogTerm || (rf.Logs[len(rf.Logs)-1].Term == args.LastLogTerm && len(rf.Logs) <= args.LastLogIndex) {
	...
	for !rf.isKilled && rf.Role == 2 {
			...
			if rf.NextIndex[i] == -1 {
				...
				go func(i int) {
					...
					if ok && !reply.Success {
						...
						//rf.mu.Lock()
						DPrintf("leader %d successfully add a match index into matchNum !", rf.me)
						rf.NextIndex[i] = -1
		...
		if len(rf.Logs) > 0 {
			go func() {
				...
				length := len(rf.Logs)-1
				rf.mu.Unlock()
				flag := 0
				for i := 0; i < length && rf.Role == 2; i++ {
					select {
						case tmp := <-matchNum:
							rf.mu.Lock()
							...
							if len(array) > len(rf.peers)/2 {
								...
								if rf.CommitIndex < array[len(rf.peers)/2] {
									...
									flag = 1
									DPrintf("update commit index to %d", rf.CommitIndex)
								}
							}
							rf.mu.Unlock()
							break
...
func (rf *Raft) Apply () {
	for !rf.isKilled {
		...
		if rf.LastApplied < rf.CommitIndex {
			DPrintf("peer %d apply message", rf.me)
			rf.LastApplied++
